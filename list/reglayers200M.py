...
class AnyNet(Backbone):
    ...
    def forward(self, x):
        outputs={}
        x=self._modules['stem'](x)
        x=self._modules['s1'](x)
        outputs['res2']=x
        x=self._modules['s2'](x)
        outputs['res3']=x
        x=self._modules['s3'](x)
        outputs['res4']=x
        x=self._modules['s4'](x)
        outputs['res5']=x
    ...
    def output_shape(self):
        ...
        return {'res2': ShapeSpec(channels=24, stride=4),
                'res3': ShapeSpec(channels=56, stride=8),
                'res4': ShapeSpec(channels=152, stride=16),
                'res5': ShapeSpec(channels=368, stride=32)}
...
# load weights from DataParallel pretrained models for cpu.
