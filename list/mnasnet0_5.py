...
class MNASNet(Backbone):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        for i in range(len(self.layers)):
            x = self.layers._modules[str(i)](x)
            if str(i) in outlist:
    ...
    def output_shape(self):
        ...
        return {'res2': ShapeSpec(channels=16, stride=4),
                'res3': ShapeSpec(channels=24, stride=8),
                'res4': ShapeSpec(channels=48, stride=16),
...
def build_mnasnet0_5_backbone(cfg, input_shape):
    return mnasnet0_5()
